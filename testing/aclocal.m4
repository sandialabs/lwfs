...
AC_ARG_WITH(portals,
	[  --with-portals=DIR      Location of Portals],
	[ ac_with_portals=yes;
...
if test x"$ac_portals_hdr_ok" = xno; then
	AC_CHECK_HEADER(portals3.h, 
		 ...
		 PORTALS_HEADER="portals3.h";
		 PORTALS_RT_HEADER="portals3.h";
		 PORTALS_NAL_HEADER="portals3.h";
...
if test x"$ac_portals_hdr_ok" = xno; then
	AC_CHECK_HEADER(portals/portals3.h, 
 		[ac_portals_hdr_ok=yes;
		 PORTALS_HEADER="portals/portals3.h";
		 PORTALS_RT_HEADER="portals/portals3.h";
		 PORTALS_NAL_HEADER="portals/portals3.h";
		 PORTALS_CFLAGS="$PORTALS_CFLAGS $EXTRA_CFLAGS";
		 PORTALS_CPPFLAGS="$PORTALS_CPPFLAGS $EXTRA_CFLAGS"],
 		[ac_portals_hdr_ok=no])
fi
...
if test x"$ac_portals_lib_ok" = xno -a x$ac_portals_hdr_ok = xyes; then
    ...
    AC_LINK_IFELSE(AC_LANG_CALL([],[PtlInit]),
		[PORTALS_LIBS=$extra_LIBS;
		 PORTALS_RT_HEADER="p3rt/p3rt.h";
		 PORTALS_NAL_HEADER="p3nal_utcp.h";
		ac_portals_lib_ok=yes;
		AC_MSG_RESULT(yes)],
		[ac_portals_lib_ok=no;
		 AC_MSG_RESULT(no)])
...
AC_DEFINE_UNQUOTED([PORTALS_HEADER], ["$PORTALS_HEADER"], 
	["Path to portals3.h"])
AC_DEFINE_UNQUOTED([PORTALS_RT_HEADER], ["$PORTALS_RT_HEADER"], 
	["Path to Portals runtime definitions"])
AC_DEFINE_UNQUOTED([PORTALS_NAL_HEADER], ["$PORTALS_NAL_HEADER"], 
	["Path to portals NAL definitions"])
...
#
if test x"$ac_cv_type_ptl_time_t" = xno -a \
        x"$ac_cv_type_ptl_eq_handler_t" = xno -a \
        x"$ac_cv_func_PtlErrorStr" = xno -a \
...
dnl
dnl @version $Id: acx_mpi.m4 1117 2007-01-28 05:15:30Z raoldfi $
...
if test x = x"$MPILIBS"; then
        ...
        ifelse([$1],,[AM_CONDITIONAL(HAVE_MPI,true)],[$1])
        :
fi
])dnl ACX_MPI
