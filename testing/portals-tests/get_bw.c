...
/*  This Portals 3.0 program tests the get bandwidth.
 */
...
static 
const char usage[] = " \n\
get_bw [-dhv] [-m <min>] [-M <max>] [-i [x]<inc>] [-n <cnt>] [-o <cnt>] \n\
 \n\
	Performs a bandwidth test using PtlGet calls between pairs of \n\
	processes, where rank n/2 processes get data and rank 1+n/2 \n\
	processes send data, for 0 <= n <= N/2, with N even. \n\
 \n\
	The elapsed time clock for each message sent starts when PtlGet \n\
	is called, and ends when the data is received. \n\
 \n\
	...
	-o cnt	Sets the number of outstanding gets allowed.  Increasing \n\
		  this value from 1 allows more pipelining of messages. \n\
	-v	Causes get_bw to be verbose about the progress of \n\
		  each trial. \n\
";
...
int main(int argc, char *argv[])
{
	...
	unsigned rcvr, trials = 1000;
	unsigned inflight = 1, add_inc = 1;
	unsigned i, m, dbg = 0, verbose = 0;
	...
	for (i=0; i<inflight; i++) {
		...
		msg[i].md.options = PTL_MD_OP_GET | PTL_MD_EVENT_START_DISABLE;
		 ...
		 * values here.
		 *
		 * We could also set PTL_MD_MANAGE_REMOTE, and have
		 * the intiator specify offsets.
		 */
		msg[i].md_save = msg[i].md;

		if (rcvr) {
			/* Bind the memory descriptor we use for getting.
			 */
			...
			/* Create a match entry to send this buffer.
			 * Use the source rank and inflight message count
			 * as the match bits, and don't ignore any bits.
			 */
			ptl_match_bits_t mb = 
	 ...
	 */
	if (rcvr) {
		rc = PtlGetRegion(msg[0].md_h, 0, 0, dst_id, ptl, 0, src, 0);
		if (rc != PTL_OK) {
			fprintf(stderr, "%d:PtlGet: %s\n",
				rank, PtlErrorStr(rc));
			exit(EXIT_FAILURE);
		}
		event(msg, &ev, PTL_EVENT_REPLY_END, rank, ni_h);
	}
	else event(msg, &ev, PTL_EVENT_GET_END, rank, ni_h);
...
next_size:
	...
	while (rcvr) {
		...
		if (i < trials) {
			...
			rc = PtlGetRegion(msg[m].md_h, 0, len, 
					  dst_id, ptl, 0, mb, 0);
			if (rc != PTL_OK) {
				fprintf(stderr, "%d:PtlGet: %s\n",
					rank, PtlErrorStr(rc));
		...
		event(msg+m, &ev, PTL_EVENT_REPLY_END, rank, ni_h);
		...
		if (verbose)
			printf("%d:   REPLY: len %d trial %d msg %d "
	...
	while (!rcvr) {
		...
		event(msg+m, &ev, PTL_EVENT_GET_END, rank, ni_h);

		if (verbose)
			printf("%d:   Sent reply len %d trial %d msg %d\n",
