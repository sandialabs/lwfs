	...
	int msgs_per_md)
{
	...
	/* NOTE:  THE BUG HAPPENS IN BOTH OF THESE CASES */
	if (with_bug) {
	    /* Create a persistent match-list entry (Use PtlMEUNLINK). */
	    ...
	    /* Create a match-list entry that unlinks with the MD. */
	    rc = PtlMEAttach(ni_h, portal_index, match_id, match_bits, 0,
		    PTL_UNLINK, PTL_INS_AFTER, &me_h[i]); 
	    if (rc != PTL_OK) {
		fprintf(stderr, "PtlMEAttach: %s\n", PtlErrorStr(rc));
		exit(-1); 
	    }
	...
	if (got_unlink) {
	    ...
	    if (with_bug) {
		/* do nothing with the ME, ME is persistent */
	    }
	    else {
		/* re-attache the match-list entry that unlinks with the MD. */
		rc = PtlMEAttach(ni_h, portal_index, match_id, match_bits, 0,
			PTL_UNLINK, PTL_INS_AFTER, &me_h[i]); 
		if (rc != PTL_OK) {
		    fprintf(stderr, "PtlMEAttach: %s\n", PtlErrorStr(rc));
     ...
     * 
     * If the MD unlinks automatically (i.e., created with PTL_UNLINK)
     * PtlMEUnlink will hang the node.  It doesn't matter if the 
     * ME was created with PTL_UNLINK or PTL_RETAIN. 
     *
     */
    /* Unlink the match_entries, should also unlink the MDs */
    for (i=0; i<num_queues; i++) {
	rc = PtlMEUnlink(me_h[i]);
	if (rc != PTL_OK) {
	    fprintf(stderr, "Could not unlink MD: %s (%d)\n", 
		    PtlErrorStr(rc), rc); 
	}
    }
