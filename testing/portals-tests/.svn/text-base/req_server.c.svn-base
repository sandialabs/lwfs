	...
	int msgs_per_md)
{
	...
	md[i].options = PTL_MD_OP_PUT | PTL_MD_TRUNCATE 
	    | PTL_MD_EVENT_MANUAL_UNLINK_ENABLE; 
	...
	/* Create and attach the MD to the ME (Use PtlMDUnlink) */
	rc = PtlMDAttach(me_h[i], md[i], PTL_RETAIN, &md_h[i]); 
	if (rc != PTL_OK) {
	...
	if (got_put_start && got_put_end) {
	    ...
	    /* Time to relink the MD */
	    if (msg_counts[i] >= msgs_per_md) {

		fprintf(stderr, "Resetting MD on queue[%d]\n",i);

		/* Unlink the ME (also unlinks the MD) */
		rc = PtlMEUnlink(me_h[i]); 
		if (rc != PTL_OK) {
		    fprintf(stderr, "could not unlink MD[%d]\n",i);
		    exit(-1);
		}

		/* Re-attach match-list entry (Use PtlMEUNLINK). */
		rc = PtlMEAttach(ni_h, portal_index, match_id, match_bits, 0,
			PTL_RETAIN, PTL_INS_AFTER, &me_h[i]); 
		if (rc != PTL_OK) {
		    fprintf(stderr, "PtlMEAttach: %s\n", PtlErrorStr(rc));
	...
	if (got_unlink) {
	    ...
	    fprintf(stdout, "I SHOULD NEVER GET AN UNLINK EVENT\n");
	    exit(-1);
	}
    }
