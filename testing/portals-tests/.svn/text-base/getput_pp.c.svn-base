...
/*  This Portals 3.0 program tests the getput latency.
 */
...
static 
const char usage[] = " \n\
put_pp [-dhv] [-m <min>] [-M <max>] [-i [x]<inc>] [-n <cnt>] [-o <cnt>] \n\
 \n\
	Performs a latency test using PtlGetPut calls between pairs \n\
	of processes. \n\
 \n\
	The elapsed time clock for each message sent starts when PtlGePut \n\
	is called from the rank n/2 process, and ends when that process \n\
	receives the PTL_EVENT_GETPUT_END event for the reply message sent \n\
...
static inline
void getput(ptl_process_id_t dst_id, ptl_pt_index_t ptl,
	    ...
	    int mbits, int mbuf, int trial, int rank, int verbose)
{
	...
	rc = PtlGetPut(msg->md_h, msg->md_h, dst_id, ptl, 0, mb, 0, 0);
	if (rc != PTL_OK) {
	...
	if (verbose) 
		printf("%d:   PtlGetPut: len %d trial %d, "
		       "msg %d\n", rank, (int)len, trial, mbuf);
}
...
int main(int argc, char *argv[])
{
	...
	if (size & 1) {
		if (!rank)
			fprintf(stderr,
				"Getput_pp requires an even number of tasks.\n");
		exit(EXIT_FAILURE);
	}
	sender = !(rank & 1);
...
next_size:
	...
	while (sender && !done) {
		...
		if (i < trials) 
			getput(dst_id, ptl, msg+m, len, src, 
			       m, i, rank, verbose);
		m += 1;
		m %= inflight;
	...
	while (!sender) {
		...
		event(msg+m, &ev, PTL_EVENT_GETPUT_END, rank, ni_h);
		if (verbose)
			printf("%d:   Received/sent len %d trial %d msg %d\n",
